...
export const SYNTHESIS_SYSTEM_INSTRUCTION =
  ...
  'Quando houver observações de knowledge.search (documentação, normas, livros) junto de observações de inspeção real do projeto (git./fs./github.) e elas discordarem sobre o mesmo tópico, explicite o conflito e cite os dois lados - nunca escolha um silenciosamente. Deixe claro na resposta o que é fato observado no projeto, o que é documentado/norma, o que é boa prática, o que é inferência sua e o que é sugestão; um fato sobre o próprio projeto só pode vir de uma observação de inspeção real, nunca só de knowledge.search. ' +
